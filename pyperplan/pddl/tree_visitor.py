...
class TraversePDDLDomain(PDDLVisitor):
    ...
    def __init__(self):
        ...
        self._child_types_map = defaultdict(list)
        self._parent_types_map = defaultdict(list)
        self._inv_constants_map = defaultdict(list)
    ...
    def visit_domain_def(self, node):
        ...
        for t in self._types.values():
            ...
            self._child_types_map[t.parent.name].append(t.name)




        sorted_type = ['object']
        queue = self._child_types_map["object"].copy()
        while queue:
            t = queue.pop()
            queue += self._child_types_map[t].copy()
            sorted_type.append(t)
        while sorted_type:
            t = sorted_type.pop()
            for child in self._child_types_map[t].copy():
                self._child_types_map[t]+=self._child_types_map[child].copy()

        for t in self._types.values():
            for c in self._child_types_map[t.name]:
                self._parent_types_map[c].append(t.name)
        ...
        self.domain = pddl.Domain(
            node.name, self._types, self._predicates, self._actions, self._constants, self._child_types_map, self._inv_constants_map, self._parent_types_map
        )
...
class TraversePDDLProblem(PDDLVisitor):
    ...
    def __init__(self, domain):
        ...
        self._nodeHash = dict()
        self._objects = dict()
        self._type_object_map = defaultdict(list)
    ...
    def visit_problem_def(self, node):
        ...
        self._problemDef = pddl.Problem(
            node.name, self._domain, self._objects, init_list, goal_list, self._type_object_map
        )
    ...
    def visit_object(self, node):
        ...
        self._type_object_map[type_def.name].append(node.name)
        for t in self._domain.parent_types_map[type_def.name]:
            self._type_object_map[t].append(node.name)
